...
from pkg_utils import rgb_read, to_rgb, expand_bboxes, reset_session
        ...
        if len(preds):
            ...
            if perform_face_rec:
                if profile:
                    ...
                    recognized = recognize_face(face_encs,
                                                recognized_face_encs)
            ...
            rgb = blur_faces(rgb=rgb,
                             bboxes=preds,
                             recognized_faces=
                             recognized if perform_face_rec else None,
                             blur_mode='pixelate')
            if mark_faces:
                mark_faces_cv2(rgb, preds,
                               recognized if perform_face_rec else None)
